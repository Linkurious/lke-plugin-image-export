Feature: Annotations
  ...
  Scenario Outline: Draw a text
    ...
    And I change text to <text>
    And I open preview and wait for loading
    And I select output format <outputFormat>
    When I click download <name> <outputFormat>
    Then The export <name> contains an text from <x1>,<y1> to <x2>,<y2>
  Examples:
    | x1 | y1 | x2  | y2  | text | name | outputFormat |
    | 500 | 500 | 700 | 600 | Annotation text | text.svg | svg |
