...
let mockServer: ServerProcess;
let server: ServerProcess;

BeforeAll(async function () {
  try {
    const { server: mockServerProcess } = await startMockServer();
    const { server: httpServerProcess } = await startServer(config.PORT);
    mockServer = mockServerProcess;
